...
PV=sum(particle_conc*particle_volume)*1D9*1000
...
do while (time < time_end) ! Main program time step loop
  ...
  if (use_nucleation) then
    ! write(*,*) particle_conc(1), cond_vapour, nucleation_coef
    call Nucleation(particle_conc, particle_volume, nucleation_coef, cond_vapour, timestep)
  end if
